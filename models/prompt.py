...
class Video_Prompt_Generator(nn.Module):
    def __init__(self, prompt_re_length, device):
        super().__init__()
        self.device = device
        self.image_text_net = FeedForwardNetwork(embed_dim=768, middle_dim=1024, output_dim=768, n_layers=2, dropout=0.3)
        self.retrieval_fuse = CrossFormer(q_dim=768, k_dim=768, v_dim=768, num_layers=2, heads=8, dim_head=64, dropout=0.3).to(device)  ################################3
...
class Text_Prompt_Generator(nn.Module):
    def __init__(self, prompt_re_length, device):
        ...
        self.image_text_net = FeedForwardNetwork(embed_dim=768, middle_dim=1024, output_dim=768, n_layers=2, dropout=0.3).to(device)
        self.retrieval_fuse = CrossFormer(q_dim=768, k_dim=768, v_dim=768, num_layers=2, heads=8, dim_head=64, dropout=0.3).to(device)
        self.adaptive_prompt = LearnablePooling(768, prompt_re_length).to(device)
